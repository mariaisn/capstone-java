...
const iValueIndex = Array.from(lines).findIndex(
  (line) => line.textContent.trim() === "5",
);
...
const jValueIndex = Array.from(lines).findIndex(
  (line) => line.textContent.trim() === "2",
);
...
function updateUI() {
  ...
  if (current === 10) {
    if (callMaxIndex >= 0 && lines[callMaxIndex])
      lines[callMaxIndex].classList.add("highlight");
    if (returnIndex >= 0 && lines[returnIndex])
      lines[returnIndex].classList.add("highlight");
  }
