...
conIn.addEventListener("keydown", function (e) {
  if (e.key === "Enter") {
    const value = parseInt (conIn.value);
...
function updateUI() {
  ...
  let tAdd = parseInt(X_VAL + Y_VAL);
  let tSub = parseInt (X_VAL - Y_VAL);
  let tMul = parseInt(X_VAL * Y_VAL);
  let tDiv = parseInt(X_VAL / Y_VAL);
  let tRem = X_VAL % Y_VAL;
  let tDiv2 = parseFloat(X1_VAL / Y1_VAL);
