...
function updateHighlight() {
  ...
  out1.innerText = current >= 38 ? `Tuition will be doubled in ${FINAL_YEAR} years` : "";
  ...
  if (condData) {
    ...
    if (goingForward) {
      animateToMemory(memTuition, numTuitionCond, formatMoney(condData.tuition));
      animateToMemory(
  ...
  if (Object.prototype.hasOwnProperty.call(incSteps, current)) {
    ...
    if (goingForward) {
      ...
      animateToMemory(document.getElementById("inc-year"), memYear, String(newYear));
    } else {
