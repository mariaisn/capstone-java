...
function updateMemoryExplanation() {
  ...
  else if (current === 3) message = "The statement creates an object for performing console input and assigns the object to the reference variable named input";
...
function updateHighlight() {
  ...
  nextBtn.disabled =
    (current === 8 && !Number.isFinite(guessValue)) || current === 18;
...
nextBtn.addEventListener("click", () => {
  ...
  if ((current === 14 || current === 16) && Number.isFinite(guessValue)) {
    const result =
      current === 14 ? "Your guess is too high" : "Your guess is too low";
