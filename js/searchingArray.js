...
function updateUI() {
  ...
  if ([8, 11, 14, 17, 20].includes(current)) {
    ...
    calcOp.innerText = "=";
    calcResult.innerText = SEARCH_VALUES[idx] === SEARCH_TARGET ? "true" : "false";
  ...
  if ([9, 12, 15, 18].includes(current)) {
    ...
    if (goingForward) {
      ...
      animateToMemory(document.getElementById("loop-inc"), memIndex, String(after));
    } else {
  ...
  if (current === 24) {
    ...
    if (goingForward) {
      out1.innerText = "";
      animateToMemory(document.getElementById("found-text"), out1, "Found at index: 4");
    }
  }
